...
function containerLabelWindow(){
   $('.containerLabelSettings').click(function(){
      $('#vFrontCenterWindowSmallSettingsImgVerwaltHeadInhalt').text('Container Label settings'); 
      showCmsIsLoading();
      
      var idContainer = $(this).data('idcontainer');
  ...
  $.ajax({
    type: "POST",
    url: "admin/frontAdmin/ajax_php/ajax_kontaktformbuilder.php",
    data: {_art: 'containerLabelSetting', VCMS_POST_LANG: $('body').attr('data-lang'),_idcontainer:idContainer},
    success: function(data) {
        ...
        $('#vFrontCenterWindowSmallSettingsImgVerwaltInhalt').html(data);
           ...
           var inputs = $('#vFrontCenterWindowSmallSettingsImgVerwaltInhalt').find('input[type=text]');
         
         $.each(inputs, function(i,v){
           var idContainer = $(v).data('idcontainer');
           var lang =  $(v).data('lang');
           var value = $('#containerLabel-'+idContainer+'-'+lang+'').val();
           $(v).val(value);
...
function setContainerLabel(){
    ...
    $('body').on('click','#vFrontSaveLabelContainer',function(){

      $.each($('.labelsName'),function(i,v){
          var lang = $(v).data('lang');
          var idCont = $(v).data('idcontainer');
         
          $('#containerLabel-'+idCont+'-'+lang+'').val($(v).val());
      });
...
function saveNewKontaktformularFromBuilderNow() {
  showCmsIsLoading();
  var labels = $('.containerLabel').serialize();
  $.ajax({
    type: "POST",
    url: "admin/frontAdmin/ajax_php/ajax_kontaktformbuilder.php",
    data: {_art: 'saveNewKontaktformularFromBuilderNow', VCMS_POST_LANG: $('body').attr('data-lang'), _curName: $('#vKontaktformularBuilderSaveNameFrm').val(), _curFormJson: $('#vFrontKontaktFormBuilderJsonFormsString').val(),'labels':labels},
    success: function(data) {
      if (data == true) {
...
function saveBearKontaktformularFromBuilderNow(curBearFormId) {
  ...
  $.ajax({
    type: "POST",
    url: "admin/frontAdmin/ajax_php/ajax_kontaktformbuilder.php",
    data: {_art: 'saveBearKontaktformularFromBuilderNow', VCMS_POST_LANG: $('body').attr('data-lang'),labels:labels, _curBearFormId: curBearFormId, _curName: $('#vKontaktformularBuilderSaveNameFrm').val(), _curFormJson: $('#vFrontKontaktFormBuilderJsonFormsString').val()},
    success: function(data) {
      if (data == true) {
        closeCenterWindowSmallSettingsImgVerwalt();
        showKontaktFormBuilderAdminWindow();
...
function showKontaktFormBuilderAdminWindowEinstellungenWindow() {
  ...
  if ($('#vFrontKontaktFormBuilderSettingCaptchaOn').val() == 'on') {
  var  curBestaetigungCaptchaChecked = ' checked="checked"';
    
  }else{
    var  curBestaetigungCaptchaChecked = '';  
  ...
  curWinHtml += '<input'+curBestaetigungCaptchaChecked+' type="checkbox" id="vKontaktformularBuilderSettingBestaetigungCaptchaFrm" name="vKontaktformularBuilderSettingBestaetigungCaptchaFrm" value="'+$('#vFrontKontaktFormBuilderSettingBestaetigungMail').val()+'" style="margin-left:100px;" />';
  curWinHtml += '<label style="width:350px; margin-left:5px;" for="vKontaktformularBuilderSettingBestaetigungMailAktiv">Captcha</label>';
  curWinHtml += '<div class="vFrontSmallSeFrmHolderAbstand"></div>';
  
  
    curWinHtml += '<label style="width:100px;" for="vKontaktformularBuilderSettingMailFrm">Link:</label>';
  curWinHtml += '<input type="text" id="vKontaktformularBuilderSettingLinkFrm" name="vKontaktformularBuilderSettingLinkFrm" value="'+$('#vFrontKontaktFormBuilderSettingLink').val()+'" /><div class="vFrontSmallSiteNoSiteOnlyNaviShowAuswahlWindow" title="Seite auswählen" id="setId"></div>';
  curWinHtml += '<div class="vFrontSmallSeFrmHolderAbstand"></div>';
  curWinHtml += '<label style="width:100px;" for="vKontaktformularBuilderSettinggoogleCodeFrm">Google code:</label>';
  curWinHtml += '<textarea id="vKontaktformularBuilderSettinggoogleCodeFrm" name="vKontaktformularBuilderSettinggoogleCodeFrm" >'+$('#vFrontKontaktFormBuilderSettinggoogleCode').val()+'</textarea>';
  curWinHtml += '<div class="vFrontSmallSeFrmHolderAbstand"></div>';
   curWinHtml += '<label style="width:100px;" for="vFrontKontaktFormBuilderSettingDeveloperFrm">Developer code:</label>';
  curWinHtml += '<textarea id="vFrontKontaktFormBuilderSettingDeveloperFrm" name="vFrontKontaktFormBuilderSettingDeveloperFrm" >'+$('#vFrontKontaktFormBuilderSettingDeveloper').val()+'</textarea>';
...
function initKontaktFormBuilderAdminWindowEinstellungenWindow() {
  ...
  $('#vKontaktformularBuilderSettingSaveBtn').click(function() {
    ...
    $('#vFrontKontaktFormBuilderSettingLink').val($('#vKontaktformularBuilderSettingLinkFrm').val());
    $('#vFrontKontaktFormBuilderSettinggoogleCode').val($('#vKontaktformularBuilderSettinggoogleCodeFrm').val());
     $('#vFrontKontaktFormBuilderSettingDeveloper').val($('#vFrontKontaktFormBuilderSettingDeveloperFrm').val());
    
    
    if ($('#vKontaktformularBuilderSettingBestaetigungCaptchaFrm').is(':checked')) {
      $('#vFrontKontaktFormBuilderSettingCaptchaOn').val('on');
    }
    else {
      $('#vFrontKontaktFormBuilderSettingCaptchaOn').val('off');
    }
...
function showBuilderElementBearWindowOnce(curElemObj) {
  ...
  if (curElemObj.attr('data-art') == 5) {
      
      var options = curElemObj.attr('data-options').split(',');
      ...
      if(options[1] == 1){
          ...
          var checked5 = 'checked="checked"';
      }else if(options[1] == 3){
          var checked6 = 'checked="checked"';
      }else{
          var checked4 = 'checked="checked"';
      }
    ...
    curHtmlIn += '<div class="vFrontSmallSeFrmHolderAbstand"></div>';
    curHtmlIn += '<label for="vKontaktformularBuilderElemOptionsFrm">Start:</label>';
    curHtmlIn += '<input type="radio"  name="dateOption"  value="1" '+checked1+'  style="margin-right: 20px;"/><input style="width: 100px;" type="text" class="datepicker" name="date1" value="'+options[2]+'" >    <br>';
     curHtmlIn += '<label for="vKontaktformularBuilderElemOptionsFrm">Today:</label>';
    curHtmlIn += '<input type="radio"  name="dateOption"  value="2" '+checked2+'  /><br>';
     curHtmlIn += '<label for="vKontaktformularBuilderElemOptionsFrm">Tomorrow:</label>';
    curHtmlIn += '<input type="radio"  name="dateOption"  value="3" '+checked3+'  /><br>';
    curHtmlIn += '<div class="vFrontSmallSeFrmHolderAbstand"></div>';
    
    curHtmlIn += '<label for="vKontaktformularBuilderElemOptionsFrm">Start:</label>';
    curHtmlIn += '<input type="radio"  name="dateOption1"  value="1" '+checked4+'  style="margin-right: 20px;"/><input type="text" class="datepicker1" name="date2" value="'+options[3]+'" style="width: 100px;"><br>';
     curHtmlIn += '<label for="vKontaktformularBuilderElemOptionsFrm">Today:</label>';
    curHtmlIn += '<input type="radio"  name="dateOption1"  value="2" '+checked5+'  /><br>';
     curHtmlIn += '<label for="vKontaktformularBuilderElemOptionsFrm">Tomorrow:</label>';
    curHtmlIn += '<input type="radio"  name="dateOption1"  value="3" '+checked6+'  /><br>';
    curHtmlIn += '<div class="vFrontSmallSeFrmHolderAbstand"></div>';
  ...
  if (checkIsBearDataFromElemOk(curElemObj)) {
    ...
    if (curElemObj.attr('data-art') == 4) {
     ...
     if (curElemObj.attr('data-art') == 5) {
      ...
      dateOptionArr.push($('input[name=dateOption]:checked').val());
      dateOptionArr.push($('input[name=dateOption1]:checked').val());
      dateOptionArr.push($('input[name=date1]').val());
      dateOptionArr.push($('input[name=date2]').val());

      curElemObj.attr('data-options', dateOptionArr);
    }
...
function getKontaktFormBuilderAdminWindowCurFormArt(curFormDragElemId) {
  ...
  else if (curFormDragElemId == "vFrontKontaktFormBuilderDragElemIsDate") {
    return 5;
  }
}
...
function buildKontaktFormBuilderAdminWindowJsonStringByElems() {
  ...
  curJsonString += ', "SendButtonText":'+JSON.stringify($('#vFrontKontaktFormBuilderSettingSendButtonText').val());
   curJsonString += ', "Redirect":'+JSON.stringify($('#vFrontKontaktFormBuilderSettingLink').val());
    curJsonString += ', "GoogleCode":'+JSON.stringify($('#vFrontKontaktFormBuilderSettinggoogleCode').val());
     curJsonString += ', "Developer":'+JSON.stringify($('#vFrontKontaktFormBuilderSettingDeveloper').val());
...
function buildKontaktFormBuilderAdminWindowAllElemsByJsonString() {
  ...
  var containerLabels  = '';
  if($('#vFrontKontaktFormBuilderJsonContainerLabels').val() != ''){
   
       containerLabels = $.parseJSON($('#vFrontKontaktFormBuilderJsonContainerLabels').val()) ;  
  }
  ...
  $.each(allElemsObj.FormData, function(key, value) {
    ...
    curSetHtml += '<legend>Container <i class="containerLabelSettings fa fa-pencil fa-lg"  data-idcontainer="'+newCountContainer+'"></i></legend>';
    curSetHtml += '<div class="vFrontKontaktFormBuilderContainerIsSetStructDelBtn" title="Löschen"></div>';
    curSetHtml += '<div class="vFrontKontaktFormBuilderDragPlaceInner"></div>';
      

    
       $.each(langArr, function(i,v){
           
        if($.isPlainObject(containerLabels['container'+newCountContainer])){
            valueLabel = containerLabels['container'+newCountContainer][v];
        }else{
            
            if($('#containerLabel-'+newCountContainer+'-'+v+'').length > 0 ){
             valueLabel =  $('#containerLabel-'+newCountContainer+'-'+v+'').val();  
            }else{
                 valueLabel = '';
            }
        ...
        curSetHtml += '<input type="hidden" value="'+valueLabel+'" class="containerLabel" id="containerLabel-'+newCountContainer+'-'+v+'" name="containerLabel[container'+newCountContainer+']['+v+']">'; 
        });
