...
class cmsSeiten extends funktionsSammlung {
  ...
  */
  ...
  public function getBearSiteFormsOnLang($langUrlName, $curBearID) {
    ...
    if($bearSiteArrCurOL['seitlaTextUrl'] == ''){
     
        
        $query = mysql_query("SELECT * FROM vseiten WHERE seitID =  $this->dbDecode($curBearID)");
        $row = mysql_fetch_array($query);
        $bearSiteArrCurOL['seitlaTextUrl'] = $row['seitTextUrl'];

    }
    
    $bearSiteArrCurOL['seitlaTextUrl'] = $this->url_title($bearSiteArrCurOL['seitlaTextUrl']);
    
    $return .= '<input type="hidden" name="siteCurBearID" id="siteCurBearID" value="' . $bearSiteArr['seitID'] . '" />';
    $return .= '<input type="hidden" name="siteCurArtNum" id="siteCurArtNum" value="' . $bearSiteArr['seitArt'] . '" />';
    $return .= '<div class="vFrontFrmHolder">';
    $return .= '<label for="siteName">Name:* (Navigation)</label><div class="vFrontLblAbstand"></div>';
    $return .= '<input maxlength="100" type="text" name="siteName" id="siteName" value="' . $bearSiteArrCurOL['seitlaName'] . '" />';
    $return .= '<div class="vFrontFrmAbstandLine"></div>';
    $return .= '<label for="siteName">Text url:</label><div class="vFrontLblAbstand"></div>';
    $return .= '<input type="text" name="seitlaTextUrl" id="seitlaTextUrl" value="' . $bearSiteArrCurOL['seitlaTextUrl'] . '" />';
    ...
    if (isset($bearSiteArr['seitArt']) && $bearSiteArr['seitArt'] != 3) {
      ...
      // *****************************************************
       ...
       $return .= '<div class="vFrontFrmAbstand"></div>';
      $return .= '<div class="vFrontFrmAbstand"></div>';
      if (isset($bearSiteArrCurOL['siteActive']) && $bearSiteArrCurOL['siteActive'] == 'no') {
        $return .= '<input type="checkbox" name="siteActive" id="siteActive" value="no" checked="checked" />';
      }else{
        $return .= '<input type="checkbox" name="siteActive" id="siteActive" value="no" />';
      }
      
      $return .= '<label class="vFrontLblCheckbox" for="siteGoogleNoIndex">Site inactive</label>';
  ...
  private function saveBearbeitSiteNowOnLangUpdate($siteID, $langId) {
    if (isset($_POST['_siteArt']) && $_POST['_siteArt'] == 3) {
      ...
      WHERE seitID = ' . $this->dbDecode($siteID) . ' AND langID = ' . $this->dbDecode($langId);
        ...
        if($_POST['_activeSite'] == ''){
            $_POST['_activeSite'] = 'yes';
        }
       $sqlSaveBearSiteText = 'UPDATE vseitelang SET  
        seitlaName = "' . $this->dbDecode($_POST['_siteName']) . '",
        seitlaTextUrl = "' . $this->dbDecode($_POST['_textUrl']) . '",
        ...
        siteActive    = "'.$this->dbDecode($_POST['_activeSite']).'"    
        WHERE seitID = ' . $this->dbDecode($siteID) . ' AND langID = ' . $this->dbDecode($langId);
    }
  ...
  private function saveBearbeitSiteNowOnLangNew($siteID, $langId) {
    if (isset($_POST['_siteArt']) && $_POST['_siteArt'] == 3) {
      $sqlSaveBearSiteText = 'INSERT INTO vseitelang 
       ...
       $sqlSaveBearSiteText = 'INSERT INTO vseitelang 
        (seitlaName,seitlaTextUrl, seitlaMetaTitle, seitlaMetaKeywords, seitlaMetaDesc, seitID, langID, seitlaCanonical, seitlaNoIndex) 
        VALUES 
        ("' . $this->dbDecode($_POST['_siteName']) . '","' . $this->dbDecode($_POST['_textUrl']) . '", "' . $this->dbDecode($_POST['_siteMetaTitle']) . '", "' . $this->dbDecode($_POST['_siteMetaKeywords']) . '", "' . $this->dbDecode($_POST['_siteMetaDesc']) . '", ' . $this->dbDecode($siteID) . ', ' . $this->dbDecode($langId) . ', "'.$this->dbDecode($_POST['_siteGoogleCanonical']).'", "'.$this->dbDecode($_POST['_siteGoogleNoIndex']).'")';
    }
    return $this->dbAbfragen($sqlSaveBearSiteText);
  }
  // *******************************************************************
