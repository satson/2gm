...
$picObj = new cmsBilder();


if(isset($_FILES['upl']) && $_FILES['upl']['error'] == 0){
  // Prüfen ob es die erlaubte Dateiendung hat
  if (!$picObj->checkIsImageFile($_FILES)) {
    echo '{"status":"error"}';
    exit();
  }
  ...
  if(move_uploaded_file($_FILES['upl']['tmp_name'], $curFilePath.$curFilenameSave)){
    if ($picObj->saveNewUploadPicInDatabase($curFilenameSave)) {
      $picThumbObj = new cmsPicMakeSmall();
      $picThumbObj->createPicThumb($_FILES['upl'], $curFilePath, $curFilenameSave);
      echo '{"status":"success"}';
      exit();
    }
  }
