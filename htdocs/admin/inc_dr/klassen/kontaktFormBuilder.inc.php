...
class cmsKontaktFormBuilderAdmin extends funktionsSammlung {
  ...
  public function saveNewKontaktformularFromBuilderNow($curName, $curJson) {
    $sqlText = 'INSERT INTO vkontaktformulare (koAktiv, koName, koJson, koIsImport) VALUES (1, "'.$this->dbDecode($curName).'", "'.$this->dbDecode($curJson).'", 2)';
  ...
  public function saveBearKontaktformularFromBuilderNow($curFormId, $curName, $curJson) {
    $sqlText = 'UPDATE vkontaktformulare SET koName = "'.$this->dbDecode($curName).'", koJson = "'.$this->dbDecode($curJson).'" WHERE koID = "'.$this->dbDecode($curFormId).'"';
    return $this->dbAbfragen($sqlText);
  }
